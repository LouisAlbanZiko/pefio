...
CF_FileView *cf_file_view_open(CF_File *file, uint64_t start, uint64_t size)
{
	...
	HANDLE handle_mapping = CreateFileMapping(file->handle_file, NULL, PAGE_READWRITE, high_order_size, low_order_size, NULL);
	...
	cm_unordered_set_insert(file->file_views, &view);
...
{
	for (uint64_t i = 0; i < cm_unordered_set_count(view->file->file_views); i++)
	{
		CF_FileView *_view = *((CF_FileView **)cm_unordered_set_get(view->file->file_views, i));
		if (_view == view)
		{
			cm_unordered_set_remove(view->file->file_views, i);
			break;
		}
	}
...
{
	if (view->data != NULL && view->handle_mapping != NULL)
	{
